    ...
    async def checkOutputTelQueue(self, hass):
        while True:
            if len(self.outputTelegramsQueue) != 0:
                # print(
                #     "in output queue "
                #     + str(len(self.outputTelegramsQueue))
                #     + " telegrams"
                # )
                outputTelegram = self.outputTelegramsQueue.pop(0)
                print("AVS SENT-" + outputTelegram.teltoStr())

                self.sockSend.sendto(outputTelegram.getByteView(), self.multicast_group)
...
def readInputData(hass, sockRead, ha_bus_address):
    ...
    if inputTelegram.initWithByteView(message) == True:
        ...
        if get_ip() != address[0] and str(inputTelegram.getSrcAddress()) != str(
            ha_bus_address
        ):
            ...
            hass.bus.async_fire(
                "avs_bus_event/" + str(inputTelegram.getTelAddress()),
                {
                    "address": inputTelegram.getTelAddress(),
                    "data_type": inputTelegram.getDataType(),
                    "source": inputTelegram.getSrcAddress(),
                    "data": inputTelegram.getData(),
                },
            )
...
class Telegram:
    def initWithData(
        self, telegram_type, source_address, telegram_address, data_type, data=0
    ):
    ...
    def getByteView(self):
        ...
        if (
            ...
            or self.telegram_type == TELEGRAM_TYPE_RESPONSE
        ):

            if self.telegram_type == TELEGRAM_TYPE_WRITE:
                t_type = 0x1F  # WRITE
            else:
                t_type = 0x17  # RESPONSE
    ...
    def convertFromByteView(self):
        ...
        if self.byteView[0] == 0xBF and self.byteView[len(self.byteView) - 1] == crc8(
            bytes(payloadData)
        ):
            ...
            self.telegram_address = ((0x00FF & self.byteView[3]) << 8) | self.byteView[
                4
            ]
            ...
            if (
                ...
                if self.data_type == 1 or self.data_type == 2 or self.data_type == 4:
                    ...
                    self.data = (
                        ((0x0000FF & self.byteView[8]) << 16)
                        | ((0x0000FF & self.byteView[9]) << 16)
                        | ((0x0000FF & self.byteView[10]) << 8)
                        | self.byteView[11]
                    )
                elif self.data_type == 9 or self.data_type == 10:
                    self.data = (
                        ((0x0000FF & self.byteView[8]) << 24)
                        | ((0x0000FF & self.byteView[9]) << 16)
                        | ((0x0000FF & self.byteView[10]) << 8)
                        | self.byteView[11]
                    )
                elif self.data_type == 11:
                    self.data = self.data = struct.unpack(
                        "i",
                        bytearray(
                            [
                                self.byteView[11],
