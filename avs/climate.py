...
CONF_MIN_TEMP = "setpoint_min"
CONF_MAX_TEMP = "setpoint_max"
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(
            CONF_THERMOSTAT_MODE, default=DEFAULT_THERMOSTAT_MODE_HEAT
        ): cv.string,
        ...
        vol.Optional(CONF_SETPOINT_STEP, default=DEFAULT_SETPOINT_STEP): vol.All(
            float, vol.Range(min=0, max=2)
        ),
        vol.Optional(CONF_MIN_TEMP, default=DEFAULT_MIN_TEMP): vol.All(
            int, vol.Range(min=-100, max=100)
        ),
...
def async_add_entities_config(hass, config, async_add_entities):
    ...
    device = {
        ...
        "measured_temperature_status": config.get(
            CONF_MEASURED_TEMPERATURE_STATUS_ADDRESS
        ),
        ...
        CONF_MAX_TEMP: config.get(CONF_MAX_TEMP),
        CONF_MIN_TEMP: config.get(CONF_MIN_TEMP),
...
class AVSClimate(ClimateEntity):
    ...
    def __init__(self, device, hass):
        ...
        self._setpoint_temperature = (
            (device[CONF_MAX_TEMP] - device[CONF_MIN_TEMP]) / 2
        ) + device[CONF_MIN_TEMP]
        ...
        self._setpoint_max = device[CONF_MAX_TEMP]
        self._setpoint_min = device[CONF_MIN_TEMP]
        ...
        if self.device["on_off_status"] != None:
            hass.bus.async_listen(
                "avs_bus_event/" + self.device["on_off_status"], self.getEvent
            )
        if self.device["measured_temperature_status"] != None:
            hass.bus.async_listen(
                "avs_bus_event/" + self.device["measured_temperature_status"],
                self.getEvent,
            )
        if self.device["setpoint_temperature_status"] != None:
            hass.bus.async_listen(
                "avs_bus_event/" + self.device["setpoint_temperature_status"],
                self.getEvent,
            )
        if self.device["operation_mode_status"] != None:
            hass.bus.async_listen(
                "avs_bus_event/" + self.device["operation_mode_status"], self.getEvent
            )
        if self.device["heat_value_status"] != None:
            hass.bus.async_listen(
                "avs_bus_event/" + self.device["heat_value_status"], self.getEvent
            )
        if self.device["cool_value_status"] != None:
            hass.bus.async_listen(
                "avs_bus_event/" + self.device["cool_value_status"], self.getEvent
            )
        if self.device["heat_cool_status"] != None:
            hass.bus.async_listen(
                "avs_bus_event/" + self.device["heat_cool_status"], self.getEvent
            )
        if self.device["error_status"] != None:
            hass.bus.async_listen(
                "avs_bus_event/" + self.device["error_status"], self.getEvent
            )

        if self._use_humidity == True:
            if self.device["measured_humidity_status"] != None:
                hass.bus.async_listen(
                    "avs_bus_event/" + self.device["measured_humidity_status"],
                    self.getEvent,
                )
            if self.device["setpoint_humidity_status"] != None:
                hass.bus.async_listen(
                    "avs_bus_event/" + self.device["setpoint_humidity_status"],
                    self.getEvent,
                )
        ...
        if self._use_aux_heat == True:
            if self.device["aux_heat_status"] != None:
                hass.bus.async_listen(
                    "avs_bus_event/" + self.device["aux_heat_status"], self.getEvent
                )
    ...
    def getStatuses(self, hass):
        ...
        if self.device["measured_temperature_status"] != None:
            ...
            readMeasuredTemp.initWithData(
                "read",
                int(self.device["ha_bus_address"]),
                int(self.device["measured_temperature_status"]),
                AVS_DPT6,
                0,
            )
        ...
        if self.device["setpoint_temperature_status"] != None:
            ...
            readSetpoint.initWithData(
                "read",
                int(self.device["ha_bus_address"]),
                int(self.device["setpoint_temperature_status"]),
                AVS_DPT6,
                0,
            )
        ...
        if self.device["operation_mode_status"] != None:
            ...
            readOperationMode.initWithData(
                "read",
                int(self.device["ha_bus_address"]),
                int(self.device["operation_mode_status"]),
                AVS_DPT2,
                0,
            )
        ...
        if self.device["heat_value_status"] != None:
            ...
            readHeatValue.initWithData(
                "read",
                int(self.device["ha_bus_address"]),
                int(self.device["heat_value_status"]),
                AVS_DPT2,
                0,
            )
        ...
        if self.device["cool_value_status"] != None:
            ...
            readCoolValue.initWithData(
                "read",
                int(self.device["ha_bus_address"]),
                int(self.device["cool_value_status"]),
                AVS_DPT2,
                0,
            )
        ...
        if self.device["heat_cool_status"] != None:
            ...
            readHeatCoolStatus.initWithData(
                "read",
                int(self.device["ha_bus_address"]),
                int(self.device["heat_cool_status"]),
                AVS_DPT2,
                0,
            )
        ...
        if self.device["error_status"] != None:
            ...
            readErrorStatus.initWithData(
                "read",
                int(self.device["ha_bus_address"]),
                int(self.device["error_status"]),
                AVS_DPT1,
                0,
            )
        ...
        if self._use_humidity == True:
            ...
            if self.device["measured_humidity_status"] != None:
                ...
                readMeasuredHumidity.initWithData(
                    "read",
                    int(self.device["ha_bus_address"]),
                    int(self.device["measured_humidity_status"]),
                    AVS_DPT2,
                    0,
                )
            ...
            if self.device["setpoint_humidity_status"] != None:
                ...
                readSetpointHumidity.initWithData(
                    "read",
                    int(self.device["ha_bus_address"]),
                    int(self.device["setpoint_humidity_status"]),
                    AVS_DPT2,
                    0,
                )
        ...
        if self._use_aux_heat == True:
            if self.device["aux_heat_status"] != None:
                ...
                readAuxHeatStatus.initWithData(
                    "read",
                    int(self.device["ha_bus_address"]),
                    int(self.device["aux_heat_status"]),
                    AVS_DPT1,
                    0,
                )
    ...
    async def getEvent(self, event):
        ...
        if (
            self.device["on_off_status"] != None
            and myEvent["address"] == int(self.device["on_off_status"])
            and myEvent["data_type"] == AVS_DPT1
        ):
            if myEvent["data"] == 0:
                self._is_on = False
            else:
            ...
            self.device["measured_temperature_status"] != None
            and myEvent["address"] == int(self.device["measured_temperature_status"])
            and myEvent["data_type"] == AVS_DPT6
        ):
            self._measured_temperature = myEvent["data"]
        elif (
            self.device["setpoint_temperature_status"] != None
            and myEvent["address"] == int(self.device["setpoint_temperature_status"])
            and myEvent["data_type"] == AVS_DPT6
        ):
            self._setpoint_temperature = myEvent["data"]
        elif (
            self.device["operation_mode_status"] != None
            and myEvent["address"] == int(self.device["operation_mode_status"])
            and myEvent["data_type"] == AVS_DPT2
        ):
            self._operation_mode = myEvent["data"]
        elif (
            self.device["heat_value_status"] != None
            and myEvent["address"] == int(self.device["heat_value_status"])
            and myEvent["data_type"] == AVS_DPT2
        ):
            self._heat_value = myEvent["data"]
        elif (
            self.device["cool_value_status"] != None
            and myEvent["address"] == int(self.device["cool_value_status"])
            and myEvent["data_type"] == AVS_DPT2
        ):
            self._cool_value = myEvent["data"]
        elif (
            self.device["heat_cool_status"] != None
            and myEvent["address"] == int(self.device["heat_cool_status"])
            and myEvent["data_type"] == AVS_DPT2
        ):
            self._heat_cool_status = myEvent["data"]
        elif (
            self.device["error_status"] != None
            and myEvent["address"] == int(self.device["error_status"])
            and myEvent["data_type"] == AVS_DPT1
        ):
            self._error_status = myEvent["data"]
        elif (
            self.device["measured_humidity_status"] != None
            and myEvent["address"] == int(self.device["measured_humidity_status"])
            and myEvent["data_type"] == AVS_DPT2
        ):
            self._measured_humidity = myEvent["data"]
        elif (
            self.device["setpoint_humidity_status"] != None
            and myEvent["address"] == int(self.device["setpoint_humidity_status"])
            and myEvent["data_type"] == AVS_DPT2
        ):
            self._setpoint_humidity = myEvent["data"]
        elif (
            self.device["aux_heat_status"] != None
            and myEvent["address"] == int(self.device["aux_heat_status"])
            and myEvent["data_type"] == AVS_DPT1
        ):
            if myEvent["data"] == 0:
                self.is_aux_heat_on = False
    ...
    def hvac_mode(self):
        ...
        if self._is_on is False:
            return HVACMode.OFF
        elif self.device[CONF_THERMOSTAT_MODE] == DEFAULT_THERMOSTAT_MODE_HEAT:
            return HVACMode.HEAT
        elif self.device[CONF_THERMOSTAT_MODE] == DEFAULT_THERMOSTAT_MODE_COOL:
    ...
    def supported_features(self):
        ...
        # if (
        #     self.device["operation_mode"] != None
        #     and self.device["operation_mode_status"] != None
        # ):
        #     result |= SUPPORT_PRESET_MODE
        # if (
        #     self.device["use_humidity"] == True
        #     and self.device["setpoint_humidity"] != None
        #     and self.device["setpoint_humidity_status"] != None
        # ):
        #     result |= SUPPORT_TARGET_HUMIDITY
        # if (
        #     self.device["use_aux_heat"] == True
        #     and self.device["aux_heat"] != None
        #     and self.device["aux_heat_status"] != None
        # ):
    ...
    async def async_set_temperature(self, **kwargs):
        ...
        sendSetpoint.initWithData(
            "write",
            int(self.device["ha_bus_address"]),
            int(self.device["setpoint_temperature"]),
            AVS_DPT6,
    ...
    async def async_set_preset_mode(self, preset_mode):
        ...
        sendOperationMode.initWithData(
            "write",
            int(self.device["ha_bus_address"]),
            int(self.device["operation_mode"]),
            AVS_DPT2,
            int(operation_mode),
        )
    ...
    async def async_set_hvac_mode(self, hvac_mode):
        ...
        if self.device[CONF_ON_OFF_ADDRESS] is not None:
            on_off_telegram = Telegram()
            on_off_telegram.initWithData(
                "write",
                int(self.device["ha_bus_address"]),
                int(self.device[CONF_ON_OFF_ADDRESS]),
                AVS_DPT1,
                val,
            )
            self.hass.data[DATA_AVS].sendTelegram(on_off_telegram)
        self.schedule_update_ha_state()
    ...
    #     sendHumidity.initWithData(
    #         "write",
    #         int(self.device["ha_bus_address"]),
    #         int(self.device["setpoint_humidity"]),
    #         AVS_DPT2,
    #         int(humidity),
    #     )
    ...
    async def async_turn_aux_heat_on(self):
        ...
        sendAuxHeatOn.initWithData(
            "write",
            int(self.device["ha_bus_address"]),
            int(self.device["aux_heat"]),
            AVS_DPT1,
            int(1),
        )
